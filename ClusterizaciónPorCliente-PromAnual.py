...
Agrupado=Agrupado.groupby(['Name']).agg({'TON_sum':'mean', 'FactNeto_sum':'mean', 'Ancho_CMS_std':'mean',
       'Ancho_CMS_mean':'mean', 'PartNum_nunique':'mean', 'Precio':'mean'})
...
Q1 = aux['Ancho_CMS_std'].quantile(0.25)
Q3 = aux['Ancho_CMS_std'].quantile(0.75)
IQR = Q3 - Q1
lim_inf = Q1 - 1.5 * IQR
lim_sup = Q3 + 1.5 * IQR
aux=aux[(aux['Ancho_CMS_std']>=lim_inf) & (aux['Ancho_CMS_std']<=lim_sup) ]

aux=aux[aux['FactNeto_sum']>0]
...
plot_3d(np.array(BaseFinal[['PC1','PC2','PC3']]), BaseFinal['Cluster_op'], "OPTICS")    
BaseFinal.to_excel('BaseFINAL2.xlsx')
